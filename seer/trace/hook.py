...
class AttentionTracer:
    ...
    def __init__(
        ...
        self._handles: list = []
        # RTSS pivot — per-step timing breakdown, keyed by (request_id, step)
        self._step_timing: dict[tuple[int, int], dict[str, float]] = {}
    ...
    def record_step_timing(
        ...
        """RTSS pivot: attach timing breakdown to (current_request_id, current_step).

        These values get joined onto every (layer, head, block) row for
        this step at parquet-write time. Callers should call this once
        per step; double-calls overwrite.
        """
        self._step_timing[(self.current_request_id, self.current_step)] = {
            "c_lap_us": float(c_lap_us),
            "c_io_us": float(c_io_us),
            "c_attn_us": float(c_attn_us),
            "c_ffn_us": float(c_ffn_us),
            "step_latency_us": float(step_latency_us),
        }
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        if "is_top_k" not in df.columns:
            df["is_top_k"] = 0
        # Join timing breakdown by (request_id, step). Default zero when missing.
        for col in TIMING_COLUMNS:
            df[col] = 0.0
        if self._step_timing:
            for (req_id, step), tcols in self._step_timing.items():
                mask = (df["request_id"] == req_id) & (df["step"] == step)
                for col, val in tcols.items():
                    df.loc[mask, col] = val
        cast = {
            "request_id": "int64",
            "layer_id": "int32",
        ...
        cast.update({col: "float32" for col in TIMING_COLUMNS})
