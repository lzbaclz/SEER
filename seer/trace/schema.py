"""Attention + timing trace schema.

Each row is one (request, layer, head_group, block, step) tuple with
attention statistics aggregated over heads in the group, plus the
RTSS-pivot timing-breakdown columns (per-step decomposition of the
deadline-relevant cost).
...
Timing breakdown columns (RTSS pivot, optional — older traces predate them
and the loader fills zeros). These are *per-step* totals and are therefore
duplicated across the (layer, head, block) rows of the same step; this
denormalization keeps the schema flat and the loader simple. Use
``df.drop_duplicates(['request_id', 'step'])`` before analyzing latencies.

  c_lap_us:            float32 — LAP inference time (incl. feature build)
  c_io_us:             float32 — synchronous fetch from non-HBM tier
  c_attn_us:           float32 — attention compute on the kept set
  c_ffn_us:            float32 — FFN + residual + layer norm
  step_latency_us:     float32 — total per-step decode latency (sum of above
                                 plus any other model overhead)
...
class TraceRow:
    ...
    # RTSS pivot — timing breakdown (per-step totals)
    c_lap_us: float = 0.0
    c_io_us: float = 0.0
    c_attn_us: float = 0.0
    c_ffn_us: float = 0.0
    step_latency_us: float = 0.0


def arrow_schema():
    """Canonical Arrow schema for the trace parquet files.

    Imports pyarrow lazily so analysis-only consumers (the closed-form
    schedulability bound, the eC IO-bound regime helper, etc.) can import
    :mod:`seer.trace.schema` without dragging pyarrow into the
    dependency closure.
    """
