"""Abstract KV-cache policy interface (RTSS pivot — moved from seer.eval.policies).
...
(:mod:`seer.eval.sim`) and, post-integration, as the brain behind the vLLM
...
class KVPolicy(ABC):
    ...
    def on_step_end(self, step: int, step_latency_us: float | None = None) -> None:  # noqa: B027  -- intentional default no-op
        """Optional hook called after select_to_keep at each step.

        ``step_latency_us`` lets timing-aware policies (SEER's PI
        controller, fallback) observe the realized cost of the previous
        decision. Static baselines ignore the argument.
        """
